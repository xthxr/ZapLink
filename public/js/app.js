...
const customShortCodeSuccess = document.getElementById(
  "customShortCodeSuccess",
);
...
function showFirebaseFallbackNotice(firebaseStatus) {
  ...
  showToast(
    "Firebase fallback mode is active. Some features are disabled.",
    "warning",
  );
}
...
function initializeNavigation() {
  ...
  navItems.forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      if (page) {
        // Only handle nav items with data-page
        e.preventDefault();
...
function initializeEventListeners() {
  ...
  if (sidebarToggle) {
    ...
    const isCollapsed =
      localStorage.getItem("piikme-sidebar-collapsed") === "true";
    if (isCollapsed) {
...
async function setUsername() {
  ...
  setUsernameBtn.innerHTML =
    '<i class="fas fa-spinner fa-spin"></i> Setting...';
...
async function handleDeleteAccount(e) {
  ...
  const confirmed = confirm(
    "Are you sure you want to permanently delete your account? This action cannot be undone and will delete all your links and analytics.",
  );
  if (!confirmed) return;
...
async function openCreateLinkModal() {
  ...
  if (userProfile && userProfile.username) {
    ...
    const customShortCodeHint = document.getElementById("customShortCodeHint");
    if (customShortCodeHint) {
      customShortCodeHint.textContent =
        "Set a username to create custom branded links!";
    }
  }
}
...
async function validateCustomShortCode(shortCode) {
  ...
  if (shortCode.length < 3) {
    customShortCodeError.textContent =
  ...
  validateTimeout = setTimeout(async () => {
    try {
      ...
      // Check via API
      const response = await fetch(
        `/api/check-shortcode/${encodeURIComponent(checkCode)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
...
async function handleCreateLink() {
  ...
  createLinkSubmit.innerHTML =
    '<i class="fas fa-spinner fa-spin"></i> Creating...';

  try {
    const token = await getAuthToken();
    console.log(
      "Creating link with token:",
    ...
    if (data.success) {
      ...
      // Handle unauthorized/auth expired
...
async function loadLinks() {
  try {
    ...
    console.log(
      "Active links:",
      userLinks.filter((l) => l.isActive !== false).length,
    );
    console.log(
      "Link details:",
      userLinks.map((l) => ({ shortCode: l.shortCode, isActive: l.isActive })),
    );
...
function updateStats(links) {
  ...
  const avgRate =
    activeLinks.length > 0 ? (totalClicks / activeLinks.length).toFixed(1) : 0;
  ...
  searchableContent.features = [
    {
      ...
      icon: "plus",
      action: () => openCreateLinkModal(),
    },
    {
      name: "Analytics Dashboard",
      description: "View detailed analytics",
      icon: "chart-line",
      action: () => navigateToPage("analytics"),
    },
    {
    ...
    },
    {
      ...
      description: "Create custom branded links",
      icon: "edit",
      action: () => openCreateLinkModal(),
    },
    {
    ...
    {
      name: "Light Mode",
      description: "Toggle light theme",
      icon: "sun",
...
function performGlobalSearch(query) {
  ...
  // Search pages
  results.pages = searchableContent.pages.filter(
    (page) =>
...
async function deleteLink(shortCode) {
  if (
    !confirm(
      "Are you sure you want to deactivate this link? It will be moved to Inactive section.",
    )
  ) {
    return;
  }
  ...
  try {
    ...
    await linkDoc.ref.update({
      ...
      scheduledDeletion: firebase.firestore.Timestamp.fromDate(
        permanentDeletionDate,
      ),
    });

    showToast(
      "Link deactivated. Will be permanently deleted in 15 days.",
      "success",
    );
    loadLinks();
  } catch (error) {
...
async function permanentlyDeleteInactiveLinks() {
  if (
    !confirm(
      "Are you sure you want to permanently delete ALL inactive links? This cannot be undone!",
    )
  ) {
    return;
  }
  ...
  const verified = await verifyUserBeforeAction(
    "permanently delete all inactive links",
  );
  if (!verified) {
    return;
  }
  ...
  try {
    ...
    const inactiveLinksQuery = await db
      .collection("links")
      .where("userId", "==", currentUser.uid)
    ...
    showToast(
      `Successfully deleted ${count} inactive link${count > 1 ? "s" : ""}`,
      "success",
    );
    loadLinks();
  } catch (error) {
    ...
    if (analyticsLinkSelect && links.length > 0) {
      analyticsLinkSelect.innerHTML =
        ...
        links
          .map(
            (link) =>
              `<option value="${link.shortCode}">${link.shortUrl.replace("https://", "").replace("http://", "")}</option>`,
          )
          .join("");

      // Remove previous listener if exists
      const newSelect = analyticsLinkSelect.cloneNode(true);
      analyticsLinkSelect.parentNode.replaceChild(
        newSelect,
        analyticsLinkSelect,
      );
...
async function setupAnalyticsRealtime(linkFilter) {
  ...
  if (linkFilter === "all") {
    ...
    window.analyticsUnsubscribe = db
      .collection("analytics")
      .where("shortCode", "in", shortCodes.slice(0, 10)) // Firestore 'in' limit is 10
      .onSnapshot(
        (snapshot) => {
          console.log(
            "🚀 REAL-TIME UPDATE: New click detected! Updating in 100ms...",
          );
...
async function loadAnalyticsData(linkFilter) {
  try {
    ...
    const visitorsChange = calculatePercentageChange(
      currentPeriodVisitors.size,
      previousPeriodVisitors.size,
    );
    ...
    document.getElementById("analyticsImpressions").textContent =
    ...
    document.getElementById("analyticsVisitors").textContent =
...
function renderGeographicList(data) {
  ...
  if (!data || data.length === 0) {
    container.innerHTML =
      '<p style="color: var(--text-secondary); text-align: center;">No data available</p>';
    return;
  }
...
if (document.getElementById("geoSearchInput")) {
  document.getElementById("geoSearchInput").addEventListener("input", (e) => {
    ...
    const searchResults = filteredGeoClicks.filter((click) => {
      const location = click.location || {};
      const locationStr =
        `${location.city || ""} ${location.region || ""} ${location.country || ""}`.toLowerCase();
...
async function handleBugReport(e) {
  ...
  const submitBtn =
    ...
    document.querySelector('.bug-report-modal button[type="submit"]');
  ...
  try {
    ...
    if (response.ok && data.success) {
      closeBugReport();
      showToast(
        `Bug report created successfully! Issue #${data.issueNumber}`,
        "success",
      );
