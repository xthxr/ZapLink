...
function initializeEventListeners() {
    ...
    if (createFirstBtn) createFirstBtn.addEventListener('click', openCreateLinkModal);
    if (modalClose) modalClose.addEventListener('click', closeCreateLinkModal);
    if (modalCancel) modalCancel.addEventListener('click', closeCreateLinkModal);
    if (modalOverlay) modalOverlay.addEventListener('click', closeCreateLinkModal);
    if (createLinkSubmit) createLinkSubmit.addEventListener('click', handleCreateLink);
    ...
    if (skipUsernameBtn) skipUsernameBtn.addEventListener('click', hideUsernameModal);
    ...
    if (closeBugReportModal) closeBugReportModal.addEventListener('click', closeBugReport);
    if (cancelBugReport) cancelBugReport.addEventListener('click', closeBugReport);
    if (bugReportForm) bugReportForm.addEventListener('submit', handleBugReport);
...
function handleLandingRedirect() {
    ...
    if (redirectPage) {
        ...
        setTimeout(() => {
            navigateToPage(redirectPage, true);
        }, 200);
    }
}
    ...
    if (typeof firebase !== 'undefined' && firebase.auth) {
        ...
        firebase.auth().getRedirectResult().then((result) => {
            if (result.user) {
                ...
                if (redirectPage) {
                    window.history.replaceState({}, '', '/home');
                    setTimeout(() => navigateToPage(redirectPage, true), 200);
                } else {
                    const currentPath = window.location.pathname;
                    const currentPageFromUrl = currentPath.substring(1) || 'home';
...
async function loadLinks() {
    try {
        ...
        if (userLinks.length > 0) {
            ...
            if (activeTab) currentFilter = activeTab.dataset.filter;
...
function sortLinks(sortBy) {
    ...
    let currentFilter = 'all';
    filterTabs.forEach(tab => {
        if (tab.classList.contains('active')) currentFilter = tab.dataset.filter;
    });
...
function initializeGlobalSearch() {
    searchableContent.features = [
        ...
        { name: 'QR Code Generator', description: 'Generate QR codes for links', icon: 'qrcode', action: () => navigateToPage('qr-generator') },
    ...
    searchableContent.pages = [
        ...
        { name: 'QR Generator', description: 'Generate QR codes', icon: 'qrcode', path: 'qr-generator' },
        { name: 'Bio Link', description: 'Manage your bio link page', icon: 'link', path: 'bio-link' },
    ];
}
...
function performGlobalSearch(query) {
    ...
    const results = { links: [], features: [], pages: [] };
            ...
            <div class="search-suggestion-group">
                ...
                ${results.links.map(link => `
                    <div class="search-suggestion-item" onclick="handleSuggestionClick('link', '${link.shortCode}')">
                        <div class="search-suggestion-icon link"><i class="fas fa-link"></i></div>
            ...
            <div class="search-suggestion-group">
                ...
                ${results.features.map((feature, index) => `
                    <div class="search-suggestion-item" onclick="handleSuggestionClick('feature', ${index})">
                        <div class="search-suggestion-icon feature"><i class="fas fa-${feature.icon}"></i></div>
                        <div class="search-suggestion-content">
            ...
            <div class="search-suggestion-group">
                ...
                ${results.pages.map(page => `
                    <div class="search-suggestion-item" onclick="handleSuggestionClick('page', '${page.path}')">
                        <div class="search-suggestion-icon page"><i class="fas fa-${page.icon}"></i></div>
                        <div class="search-suggestion-content">
...
function handleSuggestionClick(type, data) {
    ...
    if (type === 'link') {
        ...
        if (feature && feature.action) feature.action();
    } else if (type === 'page') {
        navigateToPage(data);
    }
}
...
function updateWeightCalculations() {
    ...
    rows.forEach((row, i) => {
        ...
        if (seenLabels.has(label.toLowerCase())) hasDuplicateLabels = true;
        seenLabels.add(label.toLowerCase());

        try { new URL(url); } catch { hasInvalidUrls = true; }
...
function initializeSplitTestEventListeners() {
    ...
    if (saveBtn) saveBtn.addEventListener('click', saveSplitTest);
    ...
    if (addVariantBtn) {
        addVariantBtn.addEventListener('click', () => {
            ...
            row.innerHTML = `
                <div style="flex: 1; min-width: 100px;">
                    <input type="text" class="form-input variant-label" value="${nextLabel}" placeholder="Label" style="width: 100%; margin: 0; padding: 6px 10px;" required>
                </div>
                <div style="flex: 3; min-width: 200px;">
...
function renderSplitTestAnalytics(isSplitTest, variants, clicks) {
    ...
    variants.forEach((v, index) => {
        ...
        row.innerHTML = `
            ...
            <td style="padding: 12px 8px; max-width: 250px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--text-secondary);" title="${escapeHtml(v.url)}">${escapeHtml(v.url)}</td>
    ...
    if (splitTestChartInstance) splitTestChartInstance.destroy();
...
function shareLink(url) {
    if (navigator.share) {
        navigator.share({ title: 'Check out this link', url }).catch(() => {});
    } else {
        copyLink(url);
    }
}
...
async function deleteLink(shortCode) {
    if (!confirm('Are you sure you want to deactivate this link? It will be moved to Inactive section.')) return;
    ...
    try {
        ...
        const response = await fetch(`/api/links/${encodeURIComponent(shortCode)}/deactivate`, {
            method: 'PUT',
            headers: { 'Authorization': `Bearer ${token}` }
        });

        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'Failed to deactivate link');
...
async function permanentlyDeleteInactiveLinks() {
    if (!confirm('Are you sure you want to permanently delete ALL inactive links? This cannot be undone!')) return;
    ...
    try {
        ...
        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'Failed to delete inactive links');
...
async function reactivateLink(shortCode) {
    if (!confirm('Do you want to reactivate this link?')) return;

    try {
        const token = await getAuthToken();
        if (!token) { showToast('Authentication required', 'error'); return; }

        const response = await fetch(`/api/links/${encodeURIComponent(shortCode)}/reactivate`, {
            method: 'PUT',
            headers: { 'Authorization': `Bearer ${token}` }
        });

        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'Failed to reactivate link');
...
async function loadAnalytics() {
    ...
    try {
        ...
        if (!response.ok) { console.error('Failed to fetch links for analytics'); return; }
...
async function loadLinkAnalytics(shortCode) {
    const analyticsLinkSelect = document.getElementById('analyticsLinkSelect');
    if (analyticsLinkSelect) analyticsLinkSelect.value = shortCode;
...
async function loadAnalyticsData(linkFilter) {
    try {
        ...
        if (linkFilter === 'all') {
            ...
            if (!response.ok) {
                updateAnalyticsUISimple(0, 0, 0, 0);
                return;
            }
            const result = await response.json();
        ...
        for (const entry of analyticsEntries) {
            ...
            if (analytics.countries) {
                Object.keys(analytics.countries).forEach(country => countries.add(country));
            }
        }
        ...
        allClickHistory.forEach(click => {
            visitorFingerprints.add(`${click.referrer}_${click.device}_${click.browser}`);
        });
...
function updateAnalyticsUISimple(impressions, clicks, ctr, visitors) {
...
function processClicksOverTime(clickHistory) {
    ...
    let granularity, formatLabel, groupKey;

    if (ageInMinutes <= 60) {
        granularity = 'minute';
        formatLabel = (date) => new Date(date).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
        groupKey = (timestamp) => { const d = new Date(timestamp); d.setSeconds(0, 0); return d.getTime(); };
    } else if (ageInMinutes <= 1440) {
        granularity = 'hour';
        formatLabel = (date) => new Date(date).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
        groupKey = (timestamp) => { const d = new Date(timestamp); d.setMinutes(0, 0, 0); return d.getTime(); };
    } else {
        granularity = 'day';
        formatLabel = (date) => new Date(date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        groupKey = (timestamp) => { const d = new Date(timestamp); d.setHours(0, 0, 0, 0); return d.getTime(); };
    }
    ...
    if (sorted.length > 0) {
        ...
        for (let t = start; t <= end; t += interval) {
            ...
            filled.push({ label: formatLabel(t), count: existing ? existing.count : 0 });
        }

        return { labels: filled.map(f => f.label), data: filled.map(f => f.count), granularity };
    }

    return { labels: sorted.map(s => formatLabel(s.timestamp)), data: sorted.map(s => s.count), granularity };
}
...
function renderReferrersChart(data) {
    ...
    if (window.referrersChartInstance) window.referrersChartInstance.destroy();
...
async function loadDetailedGeographicData() {
    try {
        ...
        if (linkFilter === 'all') {
            ...
            if (!response.ok) { renderGeoClicksTable([]); return; }
            const result = await response.json();
            analyticsEntries = result.data || [];
        } else {
            const response = await fetch(`/api/analytics/${encodeURIComponent(linkFilter)}`, {
        ...
        for (const entry of analyticsEntries) {
            ...
            if (analytics && analytics.clickHistory && Array.isArray(analytics.clickHistory)) {
                analytics.clickHistory.forEach(click => {
                    if (click.location) {
                        allGeoClicks.push({ ...click, shortCode, shortUrl: linkData.shortUrl });
                    }
                });
            }
        }
...
function updateGeoSummaryStats(clicks) {
    ...
    clicks.forEach(click => {
        if (click.location) {
            uniqueLocations.add(`${click.location.city}, ${click.location.region}`);
...
function filterGeoData(filter) {
    ...
    if (filter === 'all') filteredGeoClicks = [...allGeoClicks];
    else if (filter === 'today') filteredGeoClicks = allGeoClicks.filter(click => new Date(click.timestamp) >= today);
    else if (filter === 'week') filteredGeoClicks = allGeoClicks.filter(click => new Date(click.timestamp) >= weekAgo);
    else if (filter === 'month') filteredGeoClicks = allGeoClicks.filter(click => new Date(click.timestamp) >= monthAgo);
...
function sortGeoData(sortBy, updateSelect = true) {
    ...
    if (sortBy === 'recent') filteredGeoClicks.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
    else if (sortBy === 'oldest') filteredGeoClicks.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
...
function renderGeoClicksTable(clicks) {
    ...
    clicks.forEach((click, index) => {
        ...
        tr.appendChild(makeTd(`${location.city || 'Unknown'}, ${location.region || 'Unknown'}`, true));
        tr.appendChild(makeTd(location.city || 'Unknown'));

        const countryTd = document.createElement('td');
        countryTd.textContent = location.country || 'Unknown';
        ...
        ipCode.textContent = click.ipAddress || 'N/A';
        ...
        deviceIcon.style.cssText = 'color: var(--accent-blue); margin-right: 6px;';
        deviceTd.appendChild(deviceIcon);
        deviceTd.appendChild(document.createTextNode(click.device || 'Unknown'));
        ...
        refIcon.style.cssText = 'color: var(--accent-purple); font-size: 11px; margin-right: 6px;';
        refTd.appendChild(refIcon);
        refTd.appendChild(document.createTextNode(click.referrer || 'Direct'));
...
if (document.getElementById('geoSearchInput')) {
    document.getElementById('geoSearchInput').addEventListener('input', (e) => {
        const query = e.target.value.toLowerCase();
        if (!query) { renderGeoClicksTable(filteredGeoClicks); return; }

        const searchResults = filteredGeoClicks.filter(click => {
            const location = click.location || {};
            return `${location.city || ''} ${location.region || ''} ${location.country || ''}`.toLowerCase().includes(query) ||
                (click.ipAddress || '').toLowerCase().includes(query) ||
                (click.device || '').toLowerCase().includes(query) ||
...
if (document.getElementById('exportGeoDataBtn')) {
    document.getElementById('exportGeoDataBtn').addEventListener('click', () => {
        if (filteredGeoClicks.length === 0) { showToast('No data to export', 'warning'); return; }
        ...
        filteredGeoClicks.forEach((click, index) => {
            ...
            csvRows.push([
                ...
                `"${location.city || 'Unknown'}, ${location.region || 'Unknown'}"`,
        ...
        link.download = `piikme-geo-${new Date().toISOString().split('T')[0]}.csv`;
...
function formatDate(dateInput) {
    ...
    if (dateInput && typeof dateInput.toDate === 'function') date = dateInput.toDate();
    else if (dateInput && dateInput._seconds) date = new Date(dateInput._seconds * 1000);
...
async function handleBugReport(e) {
    ...
    if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = 'Creating Issue...'; }
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            setTimeout(() => window.open(data.issueUrl, '_blank'), 1000);
        } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (landingLoginBtn) landingLoginBtn.addEventListener('click', showLoginModal);
    if (heroGetStartedBtn) heroGetStartedBtn.addEventListener('click', showLoginModal);
    if (loginModalClose) loginModalClose.addEventListener('click', () => { loginModal.style.display = 'none'; });
